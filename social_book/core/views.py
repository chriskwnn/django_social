...
def profile(request, username):
    ...
    following_count = len(FollowersCount.objects.filter(follower=username))
    follower_count = len(FollowersCount.objects.filter(user=username))
...
def follow(request):
    if request.method == 'POST':
        ...
        if FollowersCount.objects.filter(follower=follower, user=user).first():
            ...
            follow = FollowersCount.objects.create(user=user, follower=follower)
            follow.save()
...
def search(request):
    ...
    num_res = len(username_profile_list)
    return render(request, 'search.html', {'user_profile': user_profile, 'username_profile_list': username_profile_list, 'searched_user':request.POST['username'], 'num_res':num_res})